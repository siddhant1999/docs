...
function showPayment(walletAddress, amount, apiKey, accessToken, accountId) {
...
function handlePaymentMessage(event) {
  ...
  if (event.data?.type === 'TELLER_CONNECT_SUCCESS') {
    ...
    console.log('Bank connected successfully:', accessToken, accountId);
  }
...
document.getElementById('start-payment').addEventListener('click', function() {
  ...
  showPayment(walletAddress, amount, apiKey, accessToken, accountId);
});
```
